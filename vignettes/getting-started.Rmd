...
20 taxa is incredibly manageable! Let's go ahead and run divnet. My computer has 4 cores, so I am just going to run in parallel with the `ncores` argument. If you would like to run in parallel, check that you have the packages `doParallel`, `doSNOW`, and `foreach` installed below. If not, your code will run serially and R will give you pages of warnings. 

```{r}
"doParallel" %in% rownames(installed.packages())
"doSNOW" %in% rownames(installed.packages())
"foreach" %in% rownames(installed.packages())
```
