...
If your abundance table `W` is not a phyloseq object and you want to utilize covariate information with DivNet you will need to include an `X` covariate matrix (aka your metadata) where samples are rows and variables are columns and indicate your model of interest using `formula` in the same manner illustrated above. 
