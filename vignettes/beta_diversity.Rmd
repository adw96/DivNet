...
We would be remiss not to acknowledge Mike Lee, a bioinformatics wizard who recently put together some fantastic tutorials on bioinformatics for analysing microbiome data at [link](https://astrobiomike.github.io). Mike has very kindly allowed us to distribute the phyloseq data object from this tutorial via DivNet.
Thanks so much, Mike!
...
**Note:** For the purposes of this tutorial we set the number of bootstrap iterations to 1000, 
but for your own analyses you should consider increasing this number to at least 10000. 
...
we expect to observe a test statistic as large as we do in one of about every 9 repeated experiments (~10.5%) if the null of identical median community composition across conditions holds. 
...
In this case, we observe a small p-value, which reflects (in part) the fact that Aitchison distance
...
a small p-value does not guarantee that all group centroids differ or that any groups differ in scientifically meaningful ways.**
...
but instead of fitting DivNet with a diagonal design matrix, we construct a design matrix $X$ where rows are samples (these are our replicates or aliquots if you prefer that terminology) and columns are specimens.


That is, we let the $i,j$-th entry of $X$ be $1$ if
...
For convenience, DivNet also stores the subsetted Sinha et al. data as a `phyloseq` object. We'll load that now.
...
We have already constructed the design matrix above -- it is stored as sample data inside our `phyloseq` object. Before 
we fit a DivNet model, we pull this matrix out of the `phyloseq` object and ensure it has appropriate column names.

```{r}
sample_specimen_mat <- as.matrix(sample_data(mbqc_species))
...
colnames(sample_specimen_mat) 
# Excellent! We have our specimen names as our column names. 
```
